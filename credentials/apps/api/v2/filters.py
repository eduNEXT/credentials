...
class UserCredentialFilter(django_filters.FilterSet):
    program_uuid = ProgramRelatedFilter(label="UUID of the program for which the credential was awarded")
    type = CredentialTypeFilter(label="Type of the credential (program or course-run)")
    status = django_filters.ChoiceFilter(
        choices=UserCredential._meta.get_field("status").choices, label="Status of the credential"
    )
